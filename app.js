...
const elements = {
  ...
  resonanceTitle: document.querySelector("#resonance-title"),
  resonanceContent: document.querySelector("#resonance-content"),
};
...
const TRANSLATIONS = {
  de: {
    ...
    resonanceEyebrow: "Namens-Resonanz",
    resonanceSources: "Quellen",
    resonanceAiDisclaimer: "Dieser Resonanztext wurde KI-generiert und sollte als Orientierung, nicht als abschließende Bewertung gelesen werden.",
    resonanceGenerated: "Generiert mit {model}",
    resonanceEvidence: "Evidenz: {quality}",
    resonanceConfidence: "Konfidenz: {confidence}",
    resonanceNoSources: "Keine Quellen ausgewiesen.",
    topResults: "Top 10",
    ...
    languageDataMissing:
      "Die Ähnlichkeitsdaten für diese Sprache sind noch nicht verfügbar.",
  },
  en: {
    ...
    resonanceEyebrow: "Name resonance",
    resonanceSources: "Sources",
    resonanceAiDisclaimer: "This resonance text was generated by AI and should be read as orientation, not as a final assessment.",
    resonanceGenerated: "Generated with {model}",
    resonanceEvidence: "Evidence: {quality}",
    resonanceConfidence: "Confidence: {confidence}",
    resonanceNoSources: "No sources listed.",
    topResults: "Top 10",
    ...
    languageDataMissing: "Similarity data for this language is not available yet.",
  },
};
...
function normalizeNameForPath(value) {
  return value
    .normalize("NFKD")
    .replace(/\p{M}+/gu, "")
    .normalize("NFC")
...
function prefixForPath(normalizedPathName) {
  return normalizedPathName ? normalizedPathName.slice(0, 2).padEnd(2, "_") : "__";
}
...
async function setLanguage(language, options = {}) {
  ...
  try {
    await activateLanguageDataset(language);
    const params = new URLSearchParams(window.location.search);
    const requestedName = params.get("q") || state.selectedName?.name || "";
    if (requestedName) {
      await searchName(requestedName, { updateUrl: false });
    } else {
...
async function loadLanguageDataset(language) {
  if (state.datasetCache.has(language)) {
    return state.datasetCache.get(language);
  }

  const candidates = DATASET_PATHS[language] || DATASET_PATHS.de;
  const errors = [];
  for (const basePath of candidates) {
    try {
      const [manifest, indexPayload] = await Promise.all([
        loadJson(`${basePath}/manifest.json`),
        loadJson(`${basePath}/names.json`),
      ]);
      const dataset = { basePath, manifest, indexPayload };
      state.datasetCache.set(language, dataset);
      return dataset;
    } catch (error) {
      errors.push(error);
    }
  ...
  throw new Error(t("languageDataMissing"));
}

async function activateLanguageDataset(language) {
  const dataset = await loadLanguageDataset(language);
  state.datasetBasePath = dataset.basePath;
  state.manifest = dataset.manifest;
  populateMetrics(dataset.manifest);
  populateNames(dataset.indexPayload);
  loadFavorites();
  return dataset;
}

function clearActiveDataset() {
  state.datasetBasePath = "";
  state.manifest = null;
  state.names = [];
  state.normalizedToName = new Map();
...
async function loadPrefix(metric, prefix) {
  const basePath = state.datasetBasePath || "data";
  const cacheKey = `${basePath}/${metric}/${prefix}`;
  if (!state.prefixCache.has(cacheKey)) {
    state.prefixCache.set(cacheKey, loadJson(`${basePath}/${metric}/${prefix}.json`));
  }
  return state.prefixCache.get(cacheKey);
}

function resonancePaths(entry) {
  const normalizedPathName = normalizeNameForPath(entry.name);
  if (!normalizedPathName) return [];
  const prefix = prefixForPath(normalizedPathName);
  const languagePath = state.language === "en" ? "en/" : "";
  return [
    `data/name_resonance/${languagePath}${prefix}/${normalizedPathName}.json`,
    `name_resonance/${languagePath}${prefix}/${normalizedPathName}.json`,
  ];
}

async function loadResonance(entry) {
  if (!entry) return null;
  const cacheKey = `${state.language}:${entry.normalized}`;
  if (state.resonanceCache.has(cacheKey)) {
    return state.resonanceCache.get(cacheKey);
  }

  for (const path of resonancePaths(entry)) {
    try {
      const resonance = await loadJson(path);
      state.resonanceCache.set(cacheKey, resonance);
      return resonance;
    } catch {
      // Try the next static location.
    }
  }
  state.resonanceCache.set(cacheKey, null);
  return null;
}
...
function sourceTitle(source) {
  const title = decodeHtmlText(source.title || "").trim();
  if (title) return title;
  return sourceHost(String(source.url || "")) || String(source.url || "");
}
...
function uniqueSources(sources) {
  ...
  for (const source of sources) {
    ...
    const key = `${sourceHost(source.url)}|${sourceTitle(source)}`;
...
function hideResonance() {
  ...
  if (elements.resonancePanel) {
    elements.resonancePanel.hidden = true;
  }
  if (elements.resonanceContent) {
    elements.resonanceContent.innerHTML = "";
  }
}
...
function renderResonance(resonance) {
  ...
  if (!elements.resonancePanel || !elements.resonanceContent || !elements.resonanceTitle) return;
  if (!resonance) {
    hideResonance();
    return;
  }

  const sources = Array.isArray(resonance.sources)
    ? uniqueSources(resonance.sources).slice(0, 8)
    : [];
  const evidenceQuality = String(resonance.evidence_quality || "").trim();
  const model = String(resonance.model || "").trim();
  const confidence = confidencePercent(resonance.confidence);

  elements.resonanceTitle.textContent = resonance.name
    ? `${resonance.name}: ${t("resonanceEyebrow")}`
    : t("resonanceEyebrow");
  elements.resonanceContent.innerHTML = `
    <p class="resonance-summary">${escapeHtml(resonance.display_text || resonance.short_summary || "")}</p>
    ${renderTagList(resonance.associations)}
    <div class="resonance-meta">
      ${evidenceQuality ? `<span>${escapeHtml(t("resonanceEvidence", { quality: evidenceQuality }))}</span>` : ""}
      ${confidence ? `<span>${escapeHtml(t("resonanceConfidence", { confidence }))}</span>` : ""}
      ${model ? `<span>${escapeHtml(t("resonanceGenerated", { model }))}</span>` : ""}
    </div>
    <div class="resonance-sources">
      <h3>${escapeHtml(t("resonanceSources"))}</h3>
      ${
        sources.length
          ? `<ol>${sources
              .map(
                (source) => `
                  <li>
                    <a href="${escapeHtml(source.url)}" rel="noopener noreferrer" target="_blank">${escapeHtml(sourceTitle(source))}</a>
                    ${sourceHost(source.url) ? `<span>${escapeHtml(sourceHost(source.url))}</span>` : ""}
                  </li>
                `,
              )
              .join("")}</ol>`
          : `<p>${escapeHtml(t("resonanceNoSources"))}</p>`
      }
    </div>
    <p class="resonance-disclaimer">${escapeHtml(t("resonanceAiDisclaimer"))}</p>
  `;
  elements.resonancePanel.hidden = false;
}
...
async function searchName(rawName, options = {}) {
  ...
  try {
    ...
    renderResults();
    renderResonance(await loadResonance(entry));
  } catch (error) {
    state.results = [];
    hideResonance();
...
async function bootstrap() {
  ...
  try {
    await activateLanguageDataset(state.language);
    updatePageMetadata(null);
    const params = new URLSearchParams(window.location.search);
    ...
    if (
      state.debug &&
      requestedMetric &&
      state.manifest?.metrics?.some((metric) => metric.id === requestedMetric)
    ) {
...
elements.languageButtons.forEach((button) => {
  button.addEventListener("click", async () => {
    await setLanguage(button.dataset.lang || "de");
  });
});
...
window.addEventListener("popstate", async () => {
  ...
  if (SUPPORTED_LANGUAGES.has(requestedLanguage) && requestedLanguage !== state.language) {
    await setLanguage(requestedLanguage, { persist: false });
    languageChanged = true;
  }
  const requestedName = params.get("q");
  if (requestedName && !languageChanged) {
    await searchName(requestedName, { updateUrl: false });
